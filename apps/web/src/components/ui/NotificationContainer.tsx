...
function NotificationItem({ notification, onClose }: NotificationItemProps) {
  const { type, title, message, persistent } = notification
  ...
  return (
    <div className={`p-4 border rounded-lg shadow-lg animate-in slide-in-from-right-2 ${bgColors[type]}`}>
      <div className="flex items-start">
        ...
        {!persistent && (
          <div className="ml-4 flex-shrink-0 flex">
            <button
              onClick={onClose}
              className={`inline-flex rounded-md p-1.5 focus:outline-none focus:ring-2 focus:ring-offset-2 ${textColors[type]} hover:opacity-75`}
            >
